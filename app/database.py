...
def add_worker(fullname: str, workshop: str) -> bool:
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("INSERT INTO workers (fullname, workshop) VALUES (?, ?)", (fullname, workshop))
        conn.commit()
        conn.close()
        return True
...
def get_workers_by_workshop(workshop: str) -> List[Dict[str, Any]]:
    try:
        ...
        cursor.execute('''
            ...
            FROM workers w
            WHERE w.workshop = ?
            AND w.id NOT IN (SELECT worker_id FROM attendance WHERE date = CURRENT_DATE)
            ORDER BY w.fullname
        ''', (workshop,))
...
def get_workers_by_workshop_date(workshop: str, date: str) -> List[Dict[str, Any]]:
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute('''
            ...
            FROM workers w
            WHERE w.workshop = ?
            AND w.id NOT IN (SELECT worker_id FROM attendance WHERE date = ?)
            ORDER BY w.fullname
        ''', (workshop, date))
        workers = [dict(row) for row in cursor.fetchall()]
        conn.close()
        ...
        logger.error(f"Error getting workers by date: {e}")
        return []
...
def get_all_workers_by_shop(workshop: str) -> List[Dict[str, Any]]:
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT id, fullname, workshop FROM workers WHERE workshop = ? ORDER BY fullname", (workshop,))
        workers = [dict(row) for row in cursor.fetchall()]
        conn.close()
        return workers
    except Exception as e:
...
def mark_present(worker_id: int, shift_hours: int, ktu: float) -> bool:
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT id FROM attendance WHERE worker_id = ? AND date = CURRENT_DATE", (worker_id,))
        if cursor.fetchone():
            conn.close()
            return False
        cursor.execute("INSERT INTO attendance (worker_id, shift_hours, ktu, status) VALUES (?, ?, ?, 'present')", (worker_id, shift_hours, ktu))
        conn.commit()
        conn.close()
        return True
    except Exception as e:
...
def mark_present_date(worker_id: int, shift_hours: int, ktu: float, date: str) -> bool:
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT id FROM attendance WHERE worker_id = ? AND date = ?", (worker_id, date))
        if cursor.fetchone():
            conn.close()
            return False
        cursor.execute("INSERT INTO attendance (worker_id, date, shift_hours, ktu, status) VALUES (?, ?, ?, ?, 'present')", (worker_id, date, shift_hours, ktu))
        conn.commit()
        conn.close()
        return True
    except Exception as e:
        logger.error(f"Error marking present date: {e}")
        return False
...
def mark_other(worker_id: int, status: str) -> bool:
    try:
        ...
        if status not in valid_statuses:
            conn.close()
            return False
        cursor.execute("INSERT INTO attendance (worker_id, shift_hours, ktu, status) VALUES (?, 0, 0, ?)", (worker_id, status))
        conn.commit()
        conn.close()
        return True
    except Exception as e:
...
def mark_other_date(worker_id: int, status: str, date: str) -> bool:
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT id FROM attendance WHERE worker_id = ? AND date = ?", (worker_id, date))
        ...
        if status not in valid_statuses:
            conn.close()
            return False
        cursor.execute("INSERT INTO attendance (worker_id, date, shift_hours, ktu, status) VALUES (?, ?, 0, 0, ?)", (worker_id, date, status))
        conn.commit()
        conn.close()
        return True
    except Exception as e:
        logger.error(f"Error marking other date: {e}")
        return False
...
def get_attendance_report() -> List[Dict[str, Any]]:
    try:
        ...
        cursor.execute('''
            ...
            WHERE a.date = CURRENT_DATE ORDER BY w.workshop, w.fullname
        ''')
...
def get_attendance_report_date(date: str) -> List[Dict[str, Any]]:
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute('''
            SELECT w.fullname, w.workshop, a.status, a.ktu, a.shift_hours, a.date
            FROM attendance a JOIN workers w ON a.worker_id = w.id
            WHERE a.date = ? ORDER BY w.workshop, w.fullname
        ''', (date,))
        report = [dict(row) for row in cursor.fetchall()]
        conn.close()
        return report
    except Exception as e:
        logger.error(f"Error getting report by date: {e}")
        return []
